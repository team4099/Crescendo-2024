...
class FollowPathPlannerPathCommand(val drivetrain: Drivetrain, val path: PathPlannerPath) : Command() {
...
//    )
    ...
    override fun execute() {
        if (thetakP.hasChanged() || thetakI.hasChanged() || thetakD.hasChanged()) {
            rotationPID = PathPlannerRotationPID(thetakP.get(), thetakI.get(), thetakD.get())
            swerveDriveController = PathPlannerHolonomicDriveController(
                translationPID,
                rotationPID,
                DrivetrainConstants.MAX_AUTO_VEL,
                Translation2d(DrivetrainConstants.DRIVETRAIN_LENGTH / 2, DrivetrainConstants.DRIVETRAIN_WIDTH / 2).magnitude.meters,
